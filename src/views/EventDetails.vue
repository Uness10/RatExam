    ...
    <div class="event-detail max-w-4xl mx-auto p-6 bg-white shadow-lg rounded-lg overflow-hidden animate-fadeIn">
      ...
      <section class="event-info p-6">
        ...
        <div v-if="userHasVoted !== -1">
          ...
          <div v-else>
            <div class="alert alert-warning flex items-center justify-center p-4 mb-4 text-yellow-800 border border-yellow-300 rounded bg-yellow-50" role="alert">
              <svg class="w-6 h-6 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
              </svg>
              <span>You have already voted for this event.</span>
            </div>
          </div>
        </div>
      </section>
  ...
  export default {
    ...
    methods: {
      async loadEventData() {
        ...
        await this.checkIfUserHasVoted(eventId);
      },
      async vote(type) {
        let data ; 
        if(type==='yes'){
